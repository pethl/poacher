# app/services/picksheet_manifest_pdf_service.rb
class PicksheetManifestPdfService
  require 'prawn'

  def initialize(grouped_items, product_names, product_to_business_names, scope_label: nil)
    @grouped_items = grouped_items
    @product_names = product_names
    @product_to_business_names = product_to_business_names
    @scope_label = (scope_label.presence || "Cutting").to_s # <- default here
  ...
  def set_fonts(pdf)
    pdf.font_families.update(
      'raleway' => { normal: @raleway_font_path, bold: @raleway_bold_font_path }
    )
    pdf.font 'raleway'
  end
  ...
  def add_header(pdf)
    ...
    pdf.formatted_text(
      [
        { text: "Status: ", size: 8,  styles: [:bold], color: "555555" },
        { text: @scope_label.upcase, size: 16, styles: [:bold] }
      ],
      align: :left
    )
  ...
  def add_customers_per_product_table(pdf)
    ...
    body_rows = (0...max_rows).map { |i| @product_names.map { |p| @product_to_business_names[p][i] || "" } }
